            ...
            {enhancedFilteredIssues.length === 0 ? (
              ...
              <>
                    ...
                    >
                      <div className="p-4">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                          <span className="font-medium text-gray-700 dark:text-gray-300">PMUN: {issue.pmun || 'N/A'}</span>
                          <span className="mx-1.5">·</span>
                          <span>By: {issue.submittedBy}</span>
                        </div>
                      </div>
                      <div className="bg-gray-50 dark:bg-gray-800 px-4 py-2 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center">
                ...
                <div className="overflow-x-auto hidden md:block">
                  <div className="rounded-2xl shadow-xl border border-border bg-card overflow-auto w-full">
                    <Table className="min-w-full text-foreground text-base md:text-lg">
                      <TableHeader className="sticky top-0 z-10 bg-card/95 backdrop-blur-md">
                        <TableRow>
                          <TableHead onClick={() => handleSort('issue')} className="cursor-pointer hover:bg-blue-100/10 dark:hover:bg-blue-900/30 font-semibold text-primary text-base md:text-lg px-6 py-4 whitespace-nowrap">
                          ...
                          <TableHead onClick={() => handleSort('status')} className="cursor-pointer hover:bg-blue-100/10 dark:hover:bg-blue-900/30 font-semibold text-primary text-base md:text-lg px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">Status <SortIndicator field="status" /></div>
                          </TableHead>
                          <TableHead onClick={() => handleSort('submittedBy')} className="cursor-pointer hover:bg-blue-100/10 dark:hover:bg-blue-900/30 font-semibold text-primary text-base md:text-lg px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">Submitted By <SortIndicator field="submittedBy" /></div>
                          </TableHead>
                          <TableHead onClick={() => handleSort('reportDate')} className="cursor-pointer hover:bg-blue-100/10 dark:hover:bg-blue-900/30 font-semibold text-primary text-base md:text-lg px-6 py-4 whitespace-nowrap">
                      ...
                      <TableBody>
                        {enhancedFilteredIssues.map((issue, idx) => (
                          <TableRow
                            key={issue.id}
                            className={
                              `cursor-pointer transition-colors ${idx % 2 === 0 ? 'bg-muted/60 dark:bg-blue-900/30' : 'bg-card dark:bg-card'} hover:bg-blue-100/40 dark:hover:bg-blue-900/50`}
                          ...
                          >
                            ...
                            <TableCell className="text-center px-6 py-4 text-base md:text-lg whitespace-nowrap">
                              {issue.photos && issue.photos.length > 0 ? (
                                <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 font-semibold text-xs md:text-sm shadow-sm border border-blue-200 dark:border-blue-700">
                                  <Paperclip className="h-4 w-4 text-blue-500 dark:text-blue-300" />
                                  {issue.photos.length}
                                  <span className="sr-only">attachments</span>
                                </span>
                              ) : (
                                <span className="inline-block px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500 font-medium text-xs md:text-sm border border-gray-200 dark:border-gray-700">-</span>
                              )}
                            </TableCell>
                            <TableCell className={
                              `inline-flex items-center gap-2 px-4 py-1 rounded-full font-semibold text-xs md:text-sm shadow-sm border transition ` +
                              (issue.status === 'Open' ? 'bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 border-green-200 dark:border-green-700' :
                                issue.status === 'In Progress' ? 'bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-300 border-yellow-200 dark:border-yellow-700' :
                                issue.status === 'Completed' ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-700' :
                                issue.status === 'Escalated' ? 'bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 border-red-200 dark:border-red-700' :
                                'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700')
                            } style={{ minWidth: '110px', justifyContent: 'center' }}>
                              <span className="w-2 h-2 rounded-full inline-block" style={{
                                backgroundColor: issue.status === 'Open' ? '#22c55e' :
                                  issue.status === 'In Progress' ? '#facc15' :
                                  issue.status === 'Completed' ? '#3b82f6' :
                                  issue.status === 'Escalated' ? '#ef4444' : '#a3a3a3',
                              }}></span>
                              {issue.status}
                            </TableCell>
                            <TableCell className="px-6 py-4 text-base md:text-lg whitespace-nowrap text-foreground">{issue.submittedBy}</TableCell>
